...
export function RoleSwitcher({ callbackUrl = "/dashboard" }: RoleSwitcherProps) {
  ...
  return (
    <div className="space-y-3" data-testid="dev-role-switcher">
      ...
      <div className="quick-role-grid grid gap-2">
        {devRoleOptions().map((option) => (
          <Button
            key={option.role}
            type="button"
            variant="outline"
            className="quick-role-button h-auto justify-start gap-3 px-3 py-2 text-left"
          ...
          >
            ...
            <span className="min-w-0 flex flex-col items-start gap-0.5">
              ...
              <span className="quick-role-blurb text-xs font-normal text-muted-foreground">
                {option.blurb}
              </span>
            </span>
          </Button>
      ...
      <p className="quick-login-note text-xs text-muted-foreground">
