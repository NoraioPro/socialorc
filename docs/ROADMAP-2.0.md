...
1. **M0.6** — adapter unit tests + capability flags so P1 connectors land on a tested contract (M0.2 and M0.5 are closed).
2. **M0.3** — LinkedIn app created → connector exercised end-to-end → post on the operator's profile.
3. **M0.4** — stable OAuth origin: Vercel deploy or a named Cloudflare tunnel, replacing the ephemeral quick tunnel.
4. **P1 kickoff** — X and Facebook Page connectors built against the hardened contract.
