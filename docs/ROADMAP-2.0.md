...
| Unit test suite | **Working** | `npm run test:unit` → **41** assertions across platform-config, error-classification, content-validation, token-refresh, oauth-state |
| In-repo E2E harnesses | **Working** | `npm run test:e2e` → **18/18** (happy path, mock adapters, own server + throwaway DB) · `npm run test:durability` → **26/26** (real adapters, injected failures) · `npm run test:oauth` → **18/18** (live callback paths; needs a running server) |
...
| **Production target mismatch** | the datasource is SQLite via `better-sqlite3`, which cannot run on Vercel serverless — the PRD specifies Postgres for production | M0.4 must choose: switch to a Postgres driver adapter (schema + `TOKEN_ENCRYPTION_KEY` unchanged) or deploy the current stack to a container/VPS; decide before wiring real OAuth credentials |
